...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em parágrafor quando for apropriado.',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  - Não citar trechos e nem o contexto;
  `.trim()
